...
import { Plus, Pencil, Trash2, BookOpen, Loader2, EyeOff, Eye, X } from 'lucide-react'
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500 dark:text-slate-400">{series.length} série(s) de compréhension écrite.</p>
      ...
      {series.length === 0 ? (
        <EmptyState icon={BookOpen} title="Aucune série" description="Crée la première série CE pour que les étudiants puissent s'entraîner." />
      ) : (
...
function CeFormModal({ open, onClose, initial, userId, onSaved }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={form.id ? `Modifier la série ${form.series_number}` : 'Nouvelle série CE'} maxWidth="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label className="label">Texte à lire</label>
          <textarea required rows={6} className="input-field" value={form.passage_text} onChange={(e) => set('passage_text', e.target.value)} placeholder="Colle ici le texte de compréhension écrite..." />
        </div>
